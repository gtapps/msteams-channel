...
| A file offered in a DM shows a card with no Accept button | The bot has no app package, or its manifest is missing `supportsFiles: true`. A bot reached only through its deep link has no manifest at all: see [SETUP.md](SETUP.md#file-sending-to-channels-and-group-chats). Channel and group chat file sends are unaffected. |
