...
what is recorded; see [`SETUP.md`](SETUP.md#step-6-enable-the-channel-in-claude-code).
...
`access.json` is re-read on every inbound message, so edits take effect
...
One cause: the skill and the listener are using **different state
directories**, so the skill is editing a file nobody reads.
...
| A file offered in a DM shows a card with no Accept button | Only possible once the bot has an installed app package: set `supportsFiles: true` in its manifest. A bot reached over its deep link has no package and needs none, verified in [SETUP.md](SETUP.md#file-sending-to-channels-and-group-chats). |
| A file offer says "Something went wrong. Please try again." | Teams could not reach the channel server when Accept was clicked, almost always because the session was not running. Nothing is lost: start the session and click Allow again. Verified behaviour, and the card stays usable. |
| Clicking Accept on a file offer does nothing at all | The upload is performed by the running channel server, so it has to be up when the recipient clicks. Offers expire an hour after they are made; after that, ask Claude to send the file again. |
| A file offer still shows its buttons after Decline | Fixed by replacing the card with a "Declined" line. If it persists, the bot could not edit its own card: the offer is still cancelled, and clicking again does nothing. |
| Files fail in a channel or group chat, but work in DMs | Those routes upload to SharePoint. Either `MSTEAMS_SHAREPOINT_SITE_ID` is unset, or the app has no write grant on that site. The error text says which; see [SETUP.md](SETUP.md#file-sending-to-channels-and-group-chats). |
| Someone added to a group chat later cannot open a file sent earlier | The sharing link is scoped to the people who were in the chat when the file was sent, which is deliberate. Send it again to include them. |
